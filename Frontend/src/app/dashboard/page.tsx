...
function SlotPicker({ slots, selectedSlot, onSelect }: {
  ...
  for (const iso of displayed) {
    ...
    const key = d.toLocaleDateString("es-ES", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
        ...
        {Object.entries(byDate).map(([dateLabel, dateSlots]) => (
          <div key={dateLabel}>
            ...
            <div className="flex flex-wrap gap-2 px-4 py-3">
              {dateSlots.map((iso) => {
                const label = new Date(iso).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" });
                return (
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${BFF_URL}/api/users/${userId}`, {
      ...
      .then((res) => { if (!res.ok) throw new Error("No se pudo cargar el perfil"); return res.json(); })
      .then(setUser)
  ...
  function loadAppointments() {
    setApptLoading(true);
    setApptError("");
    const endpoint = sessionRole === "DOCTOR"
  ...
  async function onSpecialtyChange(specialty: string) {
    ...
    try {
      const [docRes, wlRes] = await Promise.all([
        ...
        fetch(`${BFF_URL}/api/waitlist/specialty/${APPT_TO_WAITLIST_SPECIALTY[specialty] ?? specialty}`, {
          headers: { Authorization: `Bearer ${sessionToken}` },
        }),
      ]);
      ...
      if (wlRes.ok) {
        ...
        const pos = wlList.findIndex((e) => e.patientId === sessionUserId && e.status === "WAITING");
  ...
  async function handleCreateAppointment(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); throw new Error(d.message ?? "Error al crear cita"); }
      setApptSuccess("Cita creada exitosamente");
      setShowApptForm(false);
      setApptSpecialty(""); setApptDoctor(""); setAvailableSlots([]); setSelectedSlot(null); setApptNotes(""); setApptType("CONSULTA");
      loadAppointments();
    } catch (err: unknown) {
  ...
  function loadWaitlist() {
    setWlLoading(true);
    setWlError("");
    const endpoint = (sessionRole === "DOCTOR" || sessionRole === "ADMIN")
  ...
  async function handleJoinWaitlist(e: React.FormEvent) {
    ...
    try {
      ...
      setShowWlForm(false);
      setWlSpecialty(""); setWlType("CONSULTA"); setWlVitalRisk(false); setWlNotes("");
      loadWaitlist();
    } catch (err: unknown) {
  ...
  async function updateVitalRisk(entryId: string, vitalRisk: boolean) {
    try {
      ...
      loadWaitlist();
    } catch { setWlError("No se pudo actualizar el riesgo vital"); }
  }
  ...
  async function cancelWaitlistEntry(entryId: string) {
    try {
      ...
      loadWaitlist();
    } catch { setWlError("No se pudo cancelar la entrada"); }
  }
  ...
  const isStaff = sessionRole === "DOCTOR" || sessionRole === "ADMIN" || sessionRole === "NURSE" || sessionRole === "RECEPTIONIST";
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 w-full">
        ...
        {loading && activeTab === "profile" && (
          <div className="bg-white rounded-2xl p-12 text-center text-gray-400 text-sm">Cargando perfil...</div>
        )}
        ...
        {activeTab === "appointments" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              {!isStaff && (
                <button onClick={() => { setShowApptForm(!showApptForm); setApptSuccess(""); setApptError(""); }}
            ...
            {apptSuccess && <Alert variant="success" message={apptSuccess} className="mb-4" />}
            {apptError && <Alert variant="error" message={apptError} className="mb-4" />}

            {/* Formulario nueva cita */}
            {showApptForm && !isStaff && (
              <div className="bg-white border border-gray-100 rounded-2xl p-6 mb-6 shadow-sm">
                <h3 className="text-sm font-bold text-gray-700 mb-4">Solicitar nueva cita</h3>
                <form onSubmit={handleCreateAppointment} className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <select value={apptSpecialty} onChange={(e) => onSpecialtyChange(e.target.value)}
                    ...
                    {waitlistPosition !== null && (
                      <p className="text-xs text-amber-600 font-semibold mt-2">
                        📋 Estás en posición #{waitlistPosition} de la lista de espera para esta especialidad
                      </p>
                    )}
                    {waitlistPosition === null && apptSpecialty && (
                      <p className="text-xs text-gray-400 mt-2">No estás en lista de espera para esta especialidad</p>
                    )}
                  </div>
                  ...
                  {apptSpecialty && (
                    <div>
                      ...
                      {doctors.length === 0 ? (
                        <p className="text-sm text-gray-400">No hay médicos disponibles para esta especialidad</p>
                      ) : (
                        <select value={apptDoctor} onChange={(e) => onDoctorChange(e.target.value)}
                          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  {apptDoctor && (
                    <div>
                      ...
                      {slotsLoading && (
                        <p className="text-sm text-gray-400">Buscando horarios disponibles...</p>
                      )}
                      {!slotsLoading && availableSlots.length === 0 && (
                        <p className="text-sm text-gray-400">No hay horarios disponibles en los próximos 6 meses</p>
                      )}
                      {!slotsLoading && availableSlots.length > 0 && (
                        <SlotPicker
                      ...
                      {selectedSlot && (
                        <p className="text-xs text-blue-700 font-semibold mt-2">
                          ✓ Seleccionado: {new Date(selectedSlot).toLocaleString("es-ES", {
                  ...
                  {apptDoctor && (
                    <>
                      ...
                      <textarea placeholder="Notas adicionales (opcional)" value={apptNotes}
                        onChange={(e) => setApptNotes(e.target.value)} rows={2}
                      ...
                      <button type="submit" disabled={apptSubmitting || !selectedSlot}
            ...
            {/* Lista de citas */}
            {apptLoading && <p className="text-sm text-gray-400">Cargando citas...</p>}
            ...
            <div className="flex flex-col gap-3">
              {appointments.map((a) => {
                ...
                return (
                  <div key={a.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${statusColors[a.status] ?? "bg-gray-100 text-gray-600"}`}>
                          {a.status}
                        </span>
                      ...
                      {a.cancelledBy && (
                        <p className="text-xs text-red-400 mt-1">Cancelada por: {a.cancelledBy}</p>
                      )}
        ...
        {activeTab === "waitlist" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              {!isStaff && (
                <button onClick={() => { setShowWlForm(!showWlForm); setWlSuccess(""); setWlError(""); }}
            ...
            {wlSuccess && <Alert variant="success" message={wlSuccess} className="mb-4" />}
            {wlError && <Alert variant="error" message={wlError} className="mb-4" />}

            {/* Formulario inscripción en lista de espera */}
            {showWlForm && !isStaff && (
              <div className="bg-white border border-gray-100 rounded-2xl p-6 mb-6 shadow-sm">
                <h3 className="text-sm font-bold text-gray-700 mb-4">Inscribirse en lista de espera</h3>
                <form onSubmit={handleJoinWaitlist} className="flex flex-col gap-4">
                  <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1 block">
                      Especialidad
                    </label>
                    <select value={wlSpecialty} onChange={(e) => setWlSpecialty(e.target.value)} required
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {(["CONSULTA", "CIRUGIA"] as const).map((t) => (
                        <button key={t} type="button" onClick={() => setWlType(t)}
                          className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-semibold border-2 transition-all ${
                            wlType === t
                  ...
                  <textarea placeholder="Motivo / notas (opcional)" value={wlNotes}
                    onChange={(e) => setWlNotes(e.target.value)} rows={2}
                  ...
                  <button type="submit" disabled={wlSubmitting || !wlSpecialty}
            ...
            {/* Lista de entradas */}
            {wlLoading && <p className="text-sm text-gray-400">Cargando lista de espera...</p>}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${priorityColors[entry.priority] ?? "bg-gray-100 text-gray-600"}`}>
                            {entry.priority}
                          </span>
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColors[entry.status] ?? "bg-gray-100 text-gray-600"}`}>
                            {entry.status}
                          </span>
                          <span className="text-xs font-semibold bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full">
                            {entry.appointmentType === "CIRUGIA" ? "🔬 Cirugía" : "🩺 Consulta"}
                          </span>
                          {entry.vitalRisk && (
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          Inscrito: {new Date(entry.createdAt).toLocaleDateString("es-ES")}
                          {entry.requeuedAt && entry.requeuedAt !== entry.createdAt && (
                            <> · Reubicado: {new Date(entry.requeuedAt).toLocaleString("es-ES")}</>
                          )}
                        </p>
                        {entry.notes && <p className="text-xs text-gray-500 mt-1">{entry.notes}</p>}

                        {/* vitalRisk toggle — solo médico o admin */}
                        {(sessionRole === "DOCTOR" || sessionRole === "ADMIN") && entry.status !== "CANCELLED" && (
                          <div className="flex items-center gap-3 mt-3">
                            <span className="text-xs font-semibold text-gray-500">Riesgo vital:</span>
                            <button onClick={() => updateVitalRisk(entry.id, !entry.vitalRisk)}
                            ...
                            <span className="text-xs text-gray-400">
                              {entry.vitalRisk ? "Activo — sube al tope de la cola" : "Inactivo"}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      {entry.status !== "CANCELLED" && (
                        <button onClick={() => cancelWaitlistEntry(entry.id)}
                          className="text-xs text-red-500 hover:text-red-700 font-semibold shrink-0 transition-colors">
                          Cancelar
                        </button>
                      )}
